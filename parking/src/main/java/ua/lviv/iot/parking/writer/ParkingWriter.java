...
public class ParkingWriter {
    private static final String CSV_HEADER = " Number,Car Number,Entry Time,Exit Time";
    ...
    public static void writeDataToCsv(List<Parking> parkings, String csvFilePath) {
        try (FileWriter writer = new FileWriter(csvFilePath)) {
            ...
            for (Parking parking : parkings) {
                writer.write(String.valueOf(parking.getAddress()));
                writer.write(CSV_SEPARATOR);
                writer.write(parking.getMarketNetwork());
                writer.write(CSV_SEPARATOR);
                writer.write(parking.getCountOfParkingSpots());
                writer.write(System.lineSeparator());
            }
